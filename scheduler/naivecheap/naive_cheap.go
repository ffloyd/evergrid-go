// Package naivecheap содержит тривиальную реализацию планировщика с акцентом на дешевое выполнение задач.
package naivecheap
...
При запросе на загрузку датасета выбираются 3 наиболее дешевых воркера с размером очереди меньше пяти,
...
выбирается наиболее дешевых с размером очереди меньше 5-и, либо просто наиболее дешевый, если это невозможно.
*/
...
func (s *Scheduler) getWorkersForUpload() []string {
	...
	sort.Sort(byPriceAsc(workersWithShortQueue))
	...
	if len(workersWithShortQueue) >= 3 {
		...
		sort.Stable(byPriceAsc(workers))
...
func (s *Scheduler) getWorkerForRun(datasetName string) string {
	...
	var result string
	if len(filtered) > 0 {
		sort.Sort(byPriceAsc(filtered))
		result = filtered[0].UID
	} else {
		sort.Sort(byPriceAsc(workers))
		sort.Stable(byQueueAsc(workers))
